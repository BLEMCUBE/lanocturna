...
<template>
	...
	<Head :title="titulo" />
	<AppLayout
		:pagina="[{ 'label': 'Envios ', link: true, url: route(ruta + '.' + tipo) }, { 'label': titulo, link: false }]">
		<div
			...
			<div class="px-2 py-0 m-0 mt-0 text-white  col-span-full  flex justify-start items-center">
				<span
					...
					<a :href="route('envios.pdf', form.id)" target="_blank"><i class="fas fa-print"></i>
						Imprimir</a>
				</span>
			</div>
			...
			<Dialog v-model:visible="isShowModal" modal header="Código Maestro" :style="{ width: '25vw' }"
				position="top" :pt="{
					header: {
				...
				<form @submit.prevent="validarCodigoMaestro">
					...
					<div class="flex justify-end py-3">
						...
						<Button label="Guardar" size="small" type="submit"
							:class="{ 'opacity-50': cod_maestro.processing }" :disabled="cod_maestro.processing" />
					</div>
				</form>
			</Dialog>
			<!--Modal codigo maestro-->

...
<template>
    <Head :title="titulo" />
    <AppLayout
        :pagina="[{ 'label': 'Bultos importados', link: true, url: route(ruta + '.bultos') }, { 'label': titulo, link: false }]">
        ...
        <Dialog v-model:visible="isShowModalProducto" modal :style="{ width: '30vw' }" :pt="{
            ...
            <p class="mb-2 font-bold">
                NO SE PUEDE EDITAR MIENTRAS EL ESTADO SEA "Arribado", DEBE DE CAMBIAR EL ESTADO A "En camino" PARA PODER
                EDITAR.
            </p>
        ...
        <div
            ...
            <div class="grid grid-cols-6 my-2 col-span-12 ">
                ...
                <div class="mx-5 col-span-6 gap-4 m-2 lg:col-span-3 flex">
                    <b>Cantidad filas importadas :</b>
                    <p>{{ deposito.depositos_detalles.length }}</p>
                </div>

            </div>

            <div class="bg-white mr-0 ml-0 p-4 mt-6  shadow rounded-lg border border-gray-300">
                <div class="bg-gray-100 rounded-t-lg py-2 px-3">
                    <h2 class="text-2xl font-medium pb-0 pl-2">Cantidad filas importadas ({{
                        deposito.depositos_detalles.length }}) </h2>
                </div>

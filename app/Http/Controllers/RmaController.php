...
class RmaController extends Controller
{
	...
	public function rma_subir()
	{
		$lista_rma = Rma::select('id', 'estado', 'nro_servicio')
			->where(function ($query) {
				$query->where('estado', '=', 'CAMBIO PRODUCTO')
					->orWhere('estado', '=', 'REPARADO');
			})->whereNot('modo', 'ENTREGADO')->get();
	...
	//subir rma a ventas para validacion
	public function subir_store(SubirRmaStoreRequest $request)
	{
		...
		//$id_rma= zero_fill("920", 5);
			...
			if(!empty($venta_query)){
			throw ValidationException::withMessages([
					'rma_existe' => __('RMA '.$id_rma." ya existe en validación" ),
				]);
			}
	...
	public function destroySubido($id)
	{
		$venta = Venta::find($id);
		if (!is_null($venta->parametro)) {
			$json_text = json_decode($venta->parametro);

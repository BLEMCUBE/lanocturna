...
class VentaController extends Controller
{
	...
	public function index()
	{
		...
		$venta_query = Venta::query()->select(
			...
			->when(Request::input('cliente') == 'true', function ($query) {
				return $query->orWhere(DB::raw('lower(cliente)'), 'LIKE', '%' . strtolower(Request::input('buscar')) . '%');
			})
			->when(Request::input('total') == 'true', function ($query) {
				return $query->orWhere(DB::raw('total'), '=', Request::input('buscar'));
			})
		...
		return Inertia::render('Venta/Index', [
			...
			'ventas' => $venta_query,
			'filtro' => Request::only(['buscar', 'inicio', 'fin', 'compra', 'cliente', 'total'])
		]);
	}
